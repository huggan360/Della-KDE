...
Item {
    ...
    property bool followAccent: false
    property color customColor1: "#2bb3a1"
    property color customColor2: "#3f7fe6"
    ...
    function reload() {
        ...
        followAccent = asBool(preferences.value("followAccent", false));
        customColor1 = preferences.value("color1", "#2bb3a1");
        customColor2 = preferences.value("color2", "#3f7fe6");
        cycleSeconds = Math.max(10, Math.min(120, Number(preferences.value("cycleSeconds", 32))));
    }
    Component.onCompleted: reload()
    // Keeps polling while hidden, so re-enabling the border in settings takes effect.
    Timer { interval: 1000; repeat: true; running: !root.Window.window || root.Window.window.visible; onTriggered: root.reload() }
