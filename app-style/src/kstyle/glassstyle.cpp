...
//______________________________________________________________
// Della radius rules: surfaces (windows, menus) use the outer radius; anything inside uses the outer
// radius minus its inset. Groups (menus, contiguous selections) round only their outer corners with
// the larger radius and use a small radius where members meet.
namespace DellaMetrics
{
constexpr qreal SurfaceRadius = 14; // windows, menus, popups (matches the decoration and KWin)
constexpr qreal ControlRadius = 8; // SurfaceRadius minus the 6px content inset
constexpr qreal JoinRadius = 4; // between members of a group (menus)
constexpr qreal ItemInset = 4; // rows sit this far inside their list/sidebar
constexpr qreal ItemRadius = ControlRadius - ItemInset; // outer corners of a row group
constexpr qreal ItemJoinRadius = 2; // where selected rows meet
}

static QPainterPath dellaRoundedPath(const QRectF &r, qreal tl, qreal tr, qreal br, qreal bl)
{
    const qreal m = std::min(r.width(), r.height()) / 2;
    tl = std::min(tl, m);
    tr = std::min(tr, m);
    br = std::min(br, m);
    bl = std::min(bl, m);
...
//______________________________________________________________
// Della: app backgrounds are painted with the same glass tint and opacity as the Della titlebars
// (~/.config/dellarc, written by Della Glass), so title bar and body read as one sheet.
static QColor dellaGlassColor(const QColor &fallback)
{
    ...
    const bool light = g.readEntry("Mode", QStringLiteral("dark")) == QLatin1String("light");
    QColor tint(g.readEntry(light ? "LightTint" : "DarkTint", light ? QStringLiteral("#f3f7f9") : QStringLiteral("#171e25")));
    tint.setAlphaF(std::clamp(g.readEntry(light ? "LightOpacity" : "DarkOpacity", light ? 0.62 : 0.5), 0.05, 1.0));
    return tint;
}

//______________________________________________________________
// Della: the style palette is derived from the current application palette each time it is needed
// (upstream cached it once, leaving widgets with stale colors after a light/dark switch).
static QPalette dellaPalette()
{
    QPalette palette = QApplication::palette();
    QColor color = palette.color(QPalette::Window);
    if (QApplication::applicationName() == QLatin1String("spectacle")) {
        color.setAlpha(127);
    } else if (color.alpha() == 0 || color.alpha() == 255) {
        color = dellaGlassColor(Qt::transparent);
    }
...
void Style::polish(QWidget *widget)
{
    ...
    widget->setPalette(dellaPalette());
    ...
    if (_isDolphin) {
        QPalette palette = QApplication::palette();
        palette.setColor(QPalette::Window, dellaGlassColor(Qt::transparent));
...
void Style::polishScrollArea(QAbstractScrollArea *scrollArea)
{
    ...
    // Della: framed views paint their rounded base in the frame; a square viewport fill on top
    // would poke out of the rounded corners.
    if (scrollArea->frameShape() != QFrame::NoFrame && scrollArea->viewport() && scrollArea->viewport()->backgroundRole() == QPalette::Base) {
...
QRect Style::subElementRect(SubElement element, const QStyleOption *option, const QWidget *widget) const
{
    ...
    // Della: row contents keep the same padding inside the (inset) row highlight.
    ...
    case SE_ItemViewItemCheckIndicator: {
        ...
        const int pad = int(DellaMetrics::ItemInset) + 4;
        const auto position = viewItemOption->viewItemPosition;
        const bool first = position == QStyleOptionViewItem::Invalid || position == QStyleOptionViewItem::Beginning || position == QStyleOptionViewItem::OnlyOne;
        const bool last = position == QStyleOptionViewItem::Invalid || position == QStyleOptionViewItem::End || position == QStyleOptionViewItem::OnlyOne;
...
bool Style::eventFilter(QObject *object, QEvent *event)
{
    if (auto dockWidget = qobject_cast<QDockWidget *>(object)) {
        ...
        // Della: widgets carry an explicit glass palette; refresh it for the new color scheme.
        const QPalette palette = dellaPalette();
...
QSize Style::itemViewItemSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *widget) const
{
    ...
    // Della: room for the row inset and text padding on both sides.
    return expandSize(size, Metrics::ItemView_ItemMarginWidth) + QSize(2 * (int(DellaMetrics::ItemInset) + 4), 0);
}
...
bool Style::drawPanelItemViewItemPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // Della grouping. Horizontally, multi-column rows only round their outer cells; vertically, a run of
    // selected rows rounds its outer corners with the control radius and meets with the join radius.
    bool roundLeft = true;
    bool roundRight = true;
    if (widget && !widget->property(PropertyNames::sidePanelView).toBool()
        && !(qobject_cast<const QTableView *>(widget) || qobject_cast<const QListWidget *>(widget) || qobject_cast<const QListView *>(widget))
        && !viewItemOption->rect.isNull()) {
        roundLeft = viewItemOption->viewItemPosition == QStyleOptionViewItem::Beginning || viewItemOption->viewItemPosition == QStyleOptionViewItem::OnlyOne;
        roundRight = viewItemOption->viewItemPosition == QStyleOptionViewItem::End || viewItemOption->viewItemPosition == QStyleOptionViewItem::OnlyOne;
    }
    qreal top = DellaMetrics::ItemRadius;
    qreal bottom = DellaMetrics::ItemRadius;
    if (selected && abstractItemView && abstractItemView->selectionModel() && viewItemOption->index.isValid()) {
        const QModelIndex index = viewItemOption->index;
        const auto isSelected = [&](int row) {
            const QModelIndex sibling = index.sibling(row, index.column());
            return sibling.isValid() && abstractItemView->selectionModel()->isSelected(sibling);
        };
        if (index.row() > 0 && isSelected(index.row() - 1)) {
            top = DellaMetrics::ItemJoinRadius;
        }
        if (isSelected(index.row() + 1)) {
            bottom = DellaMetrics::ItemJoinRadius;
        }
    }
    painter->setRenderHint(QPainter::Antialiasing);
    painter->setPen(Qt::NoPen);
    painter->setBrush(color);
    // Rows sit inside their container with padding and a 1px gap, never touching its edges.
    const qreal left = roundLeft ? DellaMetrics::ItemInset : 0;
    const qreal right = roundRight ? DellaMetrics::ItemInset : 0;
    // Item rects can be wider than the visible viewport; clip first so both sides get the same inset.
    QRect visibleRect = rect;
    if (abstractItemView && abstractItemView->viewport()) {
        const QRect viewportRect = abstractItemView->viewport()->rect();
        visibleRect.setLeft(std::max(rect.left(), viewportRect.left()));
        visibleRect.setRight(std::min(rect.right(), viewportRect.right()));
    }
    const QRectF itemRect = QRectF(visibleRect).adjusted(left, 0.5, -right, -0.5);
    painter->drawPath(dellaRoundedPath(itemRect, roundLeft ? top : 0, roundRight ? top : 0, roundRight ? bottom : 0, roundLeft ? bottom : 0));
    return true;
}
...
bool Style::drawMenuBarItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    */
    if (!menuItemOption->icon.isNull()) {
        ...
        // render outline
        if (false) { // Della: Breeze-style highlight instead of a focus underline
            QColor outlineColor;
            if (sunken)
...
bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (selected || sunken) {
        ...
        if (!useStrongFocus) {
            color.setAlphaF(sunken ? 0.4 : 0.25);
        } else if (sunken) {
        ...
        painter->setBrush(color);
        painter->setPen(Qt::NoPen);
        const QRectF itemRect = sunken ? rect.adjusted(2, 2, -2, -2) : rect.adjusted(1, 1, -1, -1);
        const qreal outer = DellaMetrics::SurfaceRadius - (Metrics::Menu_FrameWidth + 1);
        qreal top = DellaMetrics::JoinRadius;
        qreal bottom = DellaMetrics::JoinRadius;
        if (const auto menu = qobject_cast<const QMenu *>(widget)) {
            ...
            if (!first.isNull() && option->rect.top() <= first.top()) {
                top = outer;
            }
            if (!last.isNull() && option->rect.bottom() >= last.bottom()) {
                bottom = outer;
            }
        } else {
            top = bottom = outer;
        }
        painter->drawPath(dellaRoundedPath(itemRect, top, top, bottom, bottom));
        if (!useStrongFocus) {
            QColor outline = _helper->focusColor(palette);
            outline.setAlphaF(0.8);
            painter->setBrush(Qt::NoBrush);
            painter->setPen(QPen(outline, 1));
            painter->drawPath(dellaRoundedPath(itemRect.adjusted(0.5, 0.5, -0.5, -0.5), top, top, bottom, bottom));
        }
    }
