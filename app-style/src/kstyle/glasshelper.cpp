...
{
    // Della: visible hairline separators, same strength as Della's glass edges (like Breeze).
    return isDarkTheme(palette) ? QColor(255, 255, 255, 38) : QColor(0, 0, 0, 38);
}
...
void Helper::renderFrame(QPainter *painter, const QRect &rect, const QColor &color, const bool windowActive, const bool enabled) const
{
    ...
    QRectF frameRect(rect.adjusted(Metrics::Frame_FrameWidth,
                                   ...
                                   StyleConfigData::fancyMargins() ? -6 : -Metrics::Frame_FrameWidth,
                                   -Metrics::Frame_FrameWidth));
...
void Helper::renderMenuFrame(QPainter *painter, const QRect &rect, const QColor &color, const QColor &outline, bool roundCorners) const
{
    ...
    if (roundCorners) {
        painter->setRenderHint(QPainter::Antialiasing);
        QRectF frameRect(rect);
        qreal radius(14); // Della surface radius
        ...
        // outline: Della hairline, like the window edge
        const QColor edge = (outline.isValid() && outline.alpha() > 0) ? outline : (isDarkTheme(QPalette()) ? QColor(255, 255, 255, 38) : QColor(0, 0, 0, 38));
        if (edge.isValid()) {
            const QColor outline = edge;
            painter->setPen(outline);
            painter->setBrush(Qt::NoBrush);
                            ...
                            qreal animation) const
{
    ...
    if (state == CheckOff) {
        // Della: Breeze-style unchecked box: a thin outline, no fill or shadow; accent on hover.
        QColor outline = mouseOver ? palette.color(QPalette::Highlight) : palette.color(QPalette::WindowText);
        if (!mouseOver) {
